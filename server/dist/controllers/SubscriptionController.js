...
const client = new paypal.core.PayPalHttpClient(new Environment(process.env.PAYPAL_CLIENT_ID || 'client-id', process.env.PAYPAL_CLIENT_SECRET || 'client-secret'));
...
export const capturePayPalOrder = async (req, res) => {
    try {
        ...
        if (capture.result.status === 'COMPLETED') {
            ...
            user.subscriptionTier = tier;
            await user.save();
            await Transaction.findOneAndUpdate({ paymentId: orderId }, { status: 'COMPLETED' });
        ...
        console.error(error);
        if (req.body.orderId) {
            await Transaction.findOneAndUpdate({ paymentId: req.body.orderId }, { status: 'FAILED' });
        }
